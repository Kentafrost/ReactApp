...
def preview_cropped_result(video_path, coords, duration=5):
    ...
    try:
        ...
        cropped_preview = preview_clip.with_effects([crop.Crop(x1=left, y1=upper, x2=right, y2=lower)])
...
def crop_video(input_path, output_path, upper, lower, left, right):
    ...
    cropped_video = video.with_effects([crop.crop(x1=left, y1=upper, x2=right, y2=lower)])
    cropped_video = cropped_video.with_effects([resize.resize((width, height))])
