...
let csvData = 'Name,Extension,Size_MB,Video_Length,Tags\n'; // CSVヘッダー

// CSV作成用データを追加 (Add data to create CSV)
function addToCsvData(file) {
    const csvRow = `${file.name},${file.extension},${file.size_MB},${file.video_length},${file.tags.join(';')}\n`;
    csvData += csvRow;
}

// CSVファイルを作成 (Create CSV file to download it)
function saveCSV() {
    ...
    link.download = `file_data_summary_${currentDate}.csv`;
    link.click();
}
...
function createFileCard(file, index) {
    ...
    // ファイル情報セクション開始 (File information section start)
    ...
    extDiv.innerHTML = `🗂️ <strong>Extension:</strong> ${file.extension || 'Unknown'}`;
    extDiv.style.color = '#2980b9';
    infoDiv.appendChild(extDiv);
    ...
    // ファイル情報セクション終わり (File information section end)
    ...
    // ファイル名を書き換える用のHTMLファイルにリンクさせる
    const file_path = encodeURIComponent(file.path || '');
    const file_name = encodeURIComponent(file.name || '');
    
    const remarklinkDiv = document.createElement('div');
    remarklinkDiv.style.cssText = 'margin-top: 8px; text-align: right;';
    remarklinkDiv.innerHTML = `<a href="../file_name_change.html?file=${file_name}&path=${file_path}" style="color: #007bff; text-decoration: none; font-size: 12px;">✏️ Edit ${file.name || 'file'}</a>`;
    fileCard.appendChild(remarklinkDiv);
...
async function json_data_list() {
    ...
    try {
        ...
        // JSONファイルのデータをループしてカードを作成 (Loop through JSON file data to create cards)
        data.forEach((file, index) => {
            try {
                ...
                addToCsvData(file); // CSV作成用データを追加
                cardCount++;
        ...
        saveCSV(); // CSVファイル作成用の関数(invoke the function to create CSV file)
        ...
        // Vue.jsにデータを更新
        if (window.vueApp && typeof window.vueApp.updateData === 'function') {
            const allTags = [...new Set(data.flatMap(item => item.tags || []))];
            window.vueApp.updateData(data, allTags);
            console.log('Vue.js データ更新完了:', data.length, 'files,', allTags.length, 'tags');
        }
